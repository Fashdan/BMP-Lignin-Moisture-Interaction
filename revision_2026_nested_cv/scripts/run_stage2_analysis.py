"""Stage 2 forensic methodological audit for the BMP revision.

This script is diagnostic only. It reads the local analytical table, runs
repeated nested CV with the recovered Random Forest grid, compares target and
feature-set scenarios, computes residual/error summaries, prediction intervals,
family holdout stress tests, and cross-model SHAP stability outputs.

Run from the repository root:

    python stage2_outputs/run_stage2_analysis.py
"""
...
from sklearn.model_selection import GridSearchCV, KFold, LeaveOneGroupOut, ParameterGrid, RepeatedKFold
...
except Exception:  # pragma: no cover - plotting is optional for the audit tables.
    plt = None
    sns = None
...
def manual_complete_rf_grid_search(
    ...
    """Evaluate the recovered RF grid with a prefix-tree optimization.
    ...
    produce for the 50- and 100-tree candidates with the same random_state.
    This preserves the candidate grid while avoiding redundant tree fits.
    """

    candidates = list(ParameterGrid(RF_GRID))
    ...
    test_scores = [[] for _ in candidates]
    train_scores = [[] for _ in candidates]
    ...
    best_params = normalize_grid_params(candidates[best_idx])
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    (OUT / "audit_metadata.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")
...
def build_scenarios() -> list[Scenario]:
    ...
    return [
        ...
        Scenario(
            ...
            "BMP per DM",
            all_features,
            "Originally submitted BMP per DM setting; retains mathematically coupled DM and VS predictors.",
        ),
        Scenario(
            "D1_BMP_DM_no_DM",
            "BMP_DM",
            "BMP per DM",
            no_dm,
            "Single-variable exclusion sensitivity: DM removed, VS retained.",
        ),
        Scenario(
            "D2_BMP_DM_no_VS",
            "BMP_DM",
            "BMP per DM",
            no_vs,
            "Single-variable exclusion sensitivity: VS removed, DM retained.",
        ),
    ]
...
def cv_results_to_rows(
    ...
    for i in range(n):
        ...
        row = {
            ...
            "repeat": split["repeat"],
            "fold": split["fold"],
            "candidate_rank": int(cv_results["rank_test_score"][i]),
...
def run_nested_cv(
    ...
    candidates_path = TABLE_DIR / "nested_gridsearch_candidate_results_by_outer.csv"

    if force:
        for p in [pred_path, params_path, candidates_path]:
            if p.exists():
                p.unlink()
    ...
    grid_candidate_count = int(np.prod([len(v) for v in RF_GRID.values()]))
    ...
    for scenario in scenarios:
        ...
        for split in splits:
                ...
                fold_param_rows.append(
                    {
                        ...
                        "candidate_count": grid_candidate_count,
                        "best_inner_r2": float(search.best_score_),
                        "elapsed_seconds": elapsed,
                        **best_params,
                    }
                )
                fold_candidate_rows.extend(cv_results_to_rows(search.cv_results_, scenario, split, elapsed))
            ...
            append_rows(candidates_path, fold_candidate_rows)
            print(
...
def build_row_level_supplement(
    ...
    design_lines = [
        ...
        "",
        "The CSV design is saved at `stage2_outputs/predictions/final_row_level_supplementary_dataset_design.csv`.",
        "",
        ...
        "- All feature columns retain source units in their column names.",
        "",
        "Prediction fields:",
        ...
        "",
        "Full-data fitted predictions, when supplied, must be stored separately and labelled `apparent`; they are not validation predictions.",
    ]
    (ROOT / "FINAL_ROW_LEVEL_SUPPLEMENTARY_DATASET_DESIGN.md").write_text("\n".join(design_lines) + "\n", encoding="utf-8")
    return supplement


def run_family_holdout(
    filtered: pd.DataFrame, scenarios: list[Scenario], grid_jobs: int, tune_rf: bool = False
) -> pd.DataFrame:
    rows = []
    ...
    for scenario in scenarios:
        X = filtered[scenario.feature_cols].copy()
        y = filtered[scenario.target_col].astype(float).copy()
        groups = filtered["Family"].copy()
        for train_idx, test_idx in logo.split(X, y, groups=groups):
            family = str(groups.iloc[test_idx].iloc[0])
            X_train, X_test = X.iloc[train_idx], X.iloc[test_idx]
            y_train, y_test = y.iloc[train_idx], y.iloc[test_idx]

            best_score = np.nan
            best_params_json = "{}"
            model_map = fixed_models()
            if tune_rf:
                search = GridSearchCV(
                    estimator=make_pipe(RandomForestRegressor(random_state=SEED, n_jobs=1)),
                    param_grid=RF_GRID,
                ...
                best_params_json = json.dumps(normalize_grid_params(search.best_params_), sort_keys=True)
                model_map = {"RF_nested_gridsearch": search.best_estimator_, **model_map}
            for model_name, model in model_map.items():
                fitted = model if model_name == "RF_nested_gridsearch" else clone(model).fit(X_train, y_train)
                pred = fitted.predict(X_test)
                ...
                rows.append(
                    {
                        ...
                        "model": model_name,
                        "family_n": int(len(test_idx)),
                        "small_family_descriptive_only": bool(len(test_idx) < 10),
                        "best_inner_r2": best_score if model_name == "RF_nested_gridsearch" else np.nan,
                        "selected_params_json": best_params_json if model_name == "RF_nested_gridsearch" else "{}",
                        **metrics,
                    }
                )
    out = pd.DataFrame(rows)
    out.to_csv(TABLE_DIR / "leave_one_family_out_stress_test.csv", index=False)
    out.to_csv(VALIDATED_DIR / "Table_validated_leave_one_family_out_stress_test.csv", index=False)
    return out
...
def run_lignin_dm_stats(filtered: pd.DataFrame, scenarios: list[Scenario]) -> pd.DataFrame:
    ...
    out = pd.DataFrame(rows)
    out.to_csv(TABLE_DIR / "lignin_dm_hypothesis_sensitivity_stage2.csv", index=False)
...
def write_reports(
    ...
    selected = pd.read_csv(TABLE_DIR / "nested_cv_selected_hyperparameters.csv")
    candidate_count = int(np.prod([len(v) for v in RF_GRID.values()]))
    ...
    nested_lines = [
        ...
        "- Source-count reconciliation: Lallement et al. describe 132 feedstocks in narrative text, but the available analytical table contains 131 records; see `stage2_outputs/source_reconciliation/DATASET_PROVENANCE_RECONCILIATION.md`.",
        f"- Outer performance estimation: RepeatedKFold({n_splits} folds x {n_repeats} repeats, random_state=42).",
        "- Inner tuning: complete recovered RF grid with KFold(5, shuffle=True, random_state=42), scoring=`r2`, refit=True. The default runner uses a prefix-forest optimization equivalent to evaluating the 50/100/150-tree candidates; `--sklearn-gridsearch` is retained as a slower fallback.",
        "- Preprocessing is inside every sklearn Pipeline: median imputation and StandardScaler are fit only on training folds.",
        "- The submitted R2=0.557 is treated as a non-nested tuning score, not an independent validation estimate.",
        "",
        "## Recovered RF Search Space Used For Nested CV",
        f"- Candidate count per inner search: {candidate_count}.",
        ...
        "- Residual summaries are saved in `stage2_outputs/tables/nested_cv_residual_distribution_summary.csv`.",
        "- Low/mid/high observed-BMP errors are saved in `stage2_outputs/tables/nested_cv_error_by_bmp_range.csv`.",
        "- Row-level repeated out-of-fold predictions are saved in `stage2_outputs/predictions/nested_cv_row_level_oof_summary.csv`.",
        "",
        "Key RF range errors:",
        markdown_table(
        ...
        "",
        "## Tuning Provenance",
        f"- Full nested candidate results are saved in `stage2_outputs/tables/nested_gridsearch_candidate_results_by_outer.csv` ({candidate_count} candidates per outer fold).",
        "- Selected hyperparameters per outer fold are saved in `stage2_outputs/tables/nested_cv_selected_hyperparameters.csv`.",
        "- Because no contemporaneous saved original `cv_results_` artifact was found, the complete candidate table should be described as regenerated from the recovered code/grid.",
    ]
    ...
    memo_lines = [
        "# Target Selection Decision Memo",
        "",
        "## Decision Boundary",
        "This memo does not automatically replace the target, restructure the manuscript, or withdraw the Lignin-DM hypothesis. It identifies what the cleaner analyses support and what requires author approval.",
        "",
        "## Scenario Evidence",
        ...
        "- If the authors keep BMP per DM with all features, it should be labelled as the originally submitted, target-coupled predictive setting and supported by a coupling sensitivity table.",
        "- If the authors want the least coupled BMP per DM interpretation, scenario B is the conservative option.",
        "- BMP per VS remains scientifically appropriate when the claim is about methane potential normalized to volatile solids, not fresh-matter or dry-matter yield.",
        "",
        "## Author Approval Needed",
        "1. Select the primary target for the revised manuscript.",
        "2. Decide whether DM and VS remain in the main predictor set or move to sensitivity/supplementary analyses.",
        "3. Decide whether performance claims should use the nested-CV RF estimate only or include fixed ensemble baselines as co-primary comparisons.",
    ]
    (ROOT / "TARGET_SELECTION_DECISION_MEMO.md").write_text("\n".join(memo_lines) + "\n", encoding="utf-8")
    ...
    if "full_rank" in shap_outputs:
        ...
        shap_lines = [
            ...
            "",
            "## Draft-Safe Interpretation",
            "Lignin can be described as a leading predictor across RF, Gradient Boosting and Extra Trees if the relevant scenario rows above are retained. The exact ordering of oxygen, hydrogen and DM should be described as model-sensitive. The low-DM Lignin-SHAP slope contrast should remain exploratory unless authors accept the instability caveat.",
        ]
    else:
    ...
    interval_lines = [
        ...
        "",
        "## Reporting Boundary",
        "Use these intervals to communicate the spread of internal prediction errors. Do not present them as validated individual prediction intervals for deployment without external calibration or a prespecified conformal validation design.",
    ]
    (ROOT / "PREDICTION_INTERVAL_REPORT.md").write_text("\n".join(interval_lines) + "\n", encoding="utf-8")

    family_model = "RF_nested_gridsearch" if (not family.empty and (family["model"] == "RF_nested_gridsearch").any()) else "RF_fixed_selected_params"
    family_table = (
        markdown_table(
            family[family["model"] == family_model],
            ["scenario_id", "held_out_family", "family_n", "small_family_descriptive_only", "R2", "RMSE", "MAE", "bias_pred_minus_obs"],
            max_rows=50,
        )
        if not family.empty
        else "_Family holdout was not run in this invocation._"
    )
    family_lines = [
        "# Family Holdout Stress Test",
        "",
        "## Scope",
        "Leave-one-family-out validation is exploratory because some families are small, especially SLU after the DM filter. It is a stress test for domain shift, not the principal performance estimate.",
        "",
        f"## {family_model} Family Holdout",
        family_table,
        "",
        "## Interpretation",
        "Large family-specific errors should be treated as evidence that family composition affects generalization. Small-family rows are descriptive only and should not drive primary model selection.",
    ]
    (ROOT / "FAMILY_HOLDOUT_STRESS_TEST.md").write_text("\n".join(family_lines) + "\n", encoding="utf-8")
    ...
    lignin_lines = [
        "# Lignin-DM Hypothesis Positioning Notes",
        "",
        "These notes are included in the decision gate rather than inserted into the manuscript.",
        "",
        lignin_table,
    ]
    (OUT / "LIGNIN_DM_HYPOTHESIS_POSITIONING_NOTES.md").write_text("\n".join(lignin_lines) + "\n", encoding="utf-8")

    gate_lines = [
        "# Author Decision Gate 2",
        "",
        "No manuscript edits have been made.",
        "",
        "## Decisions Required Before Revision",
        "1. **Primary target:** choose among BMP per VS, conservative BMP per DM without DM/VS, or originally submitted BMP per DM with coupling disclosure.",
        "2. **Predictor set:** approve whether DM and VS remain primary predictors or become sensitivity variables.",
        "3. **Replacement performance estimate:** approve repeated nested-CV RF as the revised internal performance estimate and label the prior 0.557 as non-nested tuning performance.",
        "4. **Lignin-DM hypothesis survival:** approve retaining it as an exploratory, model-derived hypothesis only if caveats on adjusted interaction and held-out SHAP slope instability are accepted.",
        "5. **Claims withdrawn/reframed:** approve replacing predictive ceiling, robust validation, mechanism, deployment and causal biochemical wording with internally supported, sample-sensitive, model-derived association, screening and experimental-prioritization language.",
        "",
        "Claims policy table: `stage2_outputs/tables/claims_reframing_policy_stage2.csv`.",
        "",
        "## Current Evidence Snapshot",
        markdown_table(primary_rf, core_cols),
        "",
        "## Dataset Count Wording",
        "Use: Lallement et al. described a database comprising 132 feedstocks. The analytical table available and used for the present analysis contained 131 records, of which 127 met the DM >= 15% criterion.",
        "",
        "## DM Meaning",
        "DM in the source table is feedstock dry matter on a fresh-matter basis. It is not final BMP bottle solids loading and it is not industrial wet/dry digestion regime. A controlled-DM experiment remains justified only as a prospective assay-solids/hydration manipulation while controlling substrate identity, inoculum, S/I ratio, lignin contrast and block.",
        "",
        "## Data Transparency",
        "- Row-level supplement design: `FINAL_ROW_LEVEL_SUPPLEMENTARY_DATASET_DESIGN.md` and `stage2_outputs/predictions/final_row_level_supplementary_dataset_design.csv`.",
        "- Repeated out-of-fold predictions: `stage2_outputs/predictions/nested_cv_predictions_long.csv`.",
        "- Full-data/apparent predictions: `stage2_outputs/predictions/apparent_full_data_predictions_rf.csv`; these are fitted on all eligible rows and are not validation predictions.",
    ]
    (ROOT / "AUTHOR_DECISION_GATE_2.md").write_text("\n".join(gate_lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    parser.add_argument("--family-gridsearch", action="store_true")
    ...
    if args.only_nested:
        print("Nested CV checkpoint stage complete.")
        return
    ...
    if not args.skip_family:
        family = run_family_holdout(filtered, scenarios, args.grid_jobs, tune_rf=args.family_gridsearch)
    lignin_stats = pd.DataFrame()
    ...
    if not args.skip_reports:
        write_reports(meta, scenarios, summaries, family, shap_outputs, lignin_stats, args.n_repeats, args.n_splits)
